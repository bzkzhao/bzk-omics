...
| `perseus_s0` | The only test. Registry structure in place; one entry in it |
...
| **`moderated_t_ebayes`, `welch_t`** | v0.2 | Needed for the comparison capability, not for the first pipeline |
| **`ProteinAssignment` as a node** | v0.2 | v0.1 stores the candidate set and basis as fields on `SiteObservation`, which satisfies I14's display requirement without the full inference machinery |
| **`Imputation` as a node** | v0.2 | v0.1 records the parameters as JSON on `Analysis`. Sufficient for I15 |
...
MaxQuant site-table adapter. DuckDB quantitative layer. `perseus_s0` with permutation FDR. `ModifierAssignment` including supersession and retraction.
