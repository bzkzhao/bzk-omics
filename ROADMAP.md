...
- **Perseus result-table adapter** — first priority. The collaborating group's workflow, the shortest path to holding a real user's real results, and the simplest file to parse.
- Ingestion via the adapter contract, treating local and PRIDE datasets identically. Order for search-engine output: **MaxQuant** (PXD018299, the validated regression fixture), **DIA-NN** (incoming instrument-side data), **FragPipe** (hedges A2). Spectronaut on demand only. This order is correct under every plausible answer to A1 and A2, which is the point.
...
### Week 0 — the shortest useful thing
Perseus result-table adapter plus the graph. A real user's analysis output, held, connected and queryable. No statistics, no recomputation, no raw ingestion.

*Exit:* a Perseus table from a collaborator is ingested, resolved to versioned UniProt identities, and cross-queried against a second dataset.
...
DuckDB quantitative layer. MaxQuant, Spectronaut and FragPipe adapters. Two or three public interferon-stimulated diGly datasets ingested end-to-end. Sample-to-condition mapping UI, writing curation `Analysis` nodes per `ONTOLOGY.md` §5.3 — not a configuration file. SDRF parsing where present, falling back to manual curation with an explicit `basis`.
