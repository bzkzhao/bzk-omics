...
1. **`/grill-with-docs`** — sharpen the idea by interview. Start here whenever you are **working in a working directory**: it's stateful, retaining what it learns in `GLOSSARY.md` and ADRs. (No working directory? Use `/grill-me` — see Standalone. Both run the same `/grilling` primitive; `grill-with-docs` is the one that leaves a paper trail, which makes it the better of the two whenever a repo is there to leave it in.)
...
**Already satisfied.** This repo is configured; the settings the other skills read live in
`.claude/config/` — `issue-tracker.md`, `triage-labels.md`, `domain.md`. Change one by editing the
file directly. `/setup-matt-pocock-skills` is only for a structural change, such as adopting an
issue tracker or moving where decisions live.

Read `.claude/skills/README.md` for what these skills had to change to run here, and `CLAUDE.md`
for the rules that win where the two disagree.
