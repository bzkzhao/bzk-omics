...
7. **Should every `DifferentialResult` be required to attach to exactly one of `RESULT_FOR_SITE` or `RESULT_FOR_PROTEIN`?** A result measures either a site or a protein — never both, never neither — so an exactly-one (XOR) structural check would express that.
   ...
   **What remains: `ADJUSTED_BY` must become an anchor on `DifferentialResult` (§3).** That is an identity amendment and re-mints every `DifferentialResult` id, so it wants an ADR alongside the XOR invariant above. **It is free today and will not stay free** — no `DifferentialResult` exists in any graph yet, because no adapter emits one; the first ingestion of results makes this a migration. That is the argument for doing it before `perseus.py` runs in anger, not after.
