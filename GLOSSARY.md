...
**Perseus** — A desktop application for proteomics data analysis, long established as a field standard and freely available. The anchor laboratory's workflow: log₂ transformation, missing-value filtering, imputation from a normal distribution, and a modified two-sample test. Operates on one matrix per session and retains no cross-dataset model — which is the gap this platform addresses, sitting downstream of it rather than replacing it.

**s0** — A parameter in Perseus' modified *t*-test that introduces a fold-change dependence into the significance threshold, so a small change with a very small *p*-value is not called significant on statistical confidence alone. Produces the curved boundary characteristic of a Perseus volcano plot, rather than straight cutoffs on both axes. Not equivalent to a *t*-test with Benjamini–Hochberg correction.

**Permutation FDR** — False discovery rate estimated by repeatedly shuffling sample labels and counting how often the observed effect sizes arise by chance. Used by Perseus in place of Benjamini–Hochberg.

**Anchor laboratory** — The Pinto-Fernández group at the CAMS Oxford Institute: translational ubiquitomics, USP18-dependent ISGylation, cancer immune evasion. v0.1 is built for this group specifically, and scope questions resolve by asking whether they need it.

**Design headroom** — A capability the schema is structured to permit but that is not planned, built, or promised. Distinguished from a roadmap item, which is work committed to. The unbuilt `Observation` subtypes are headroom.

**Downstream positioning** — The platform consumes the outputs of search engines and analysis tools rather than replacing them. The gap it addresses is that analysis is terminal — a figure is produced and the reasoning evaporates — not that analysis is poor.

**Observed vs reported provenance** — Whether analysis parameters are known because the platform executed them, or stated because a user supplied them. Recorded as `parameters_observed` and propagated to every derived result (I19).

**External analysis** — An `Analysis` describing work performed outside the platform, typically in Perseus. Usable, and never presented with the same provenance standing as one the platform produced.

**Embargoed** — A dataset shared before publication, fully usable within the local instance but excluded from every export until released. A first-class dataset state, not a convention (I18).
