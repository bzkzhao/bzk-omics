...
**`bzk rebuild` and `bzk drift` are the names this document uses for two operations, and neither is an installed executable — stated here 2026-08-10 rather than left to be discovered.** `pyproject.toml` declares no `[project.scripts]`, so typing either at a shell finds nothing. The invocations are:

```
.venv/bin/python -m bzk.rebuild
.venv/bin/python -m bzk.drift
```

which is the form §4.1's install section and `HANDOFF.md` §3's command block already use. **Declaring console scripts was considered and rejected**, and the reason is that `bzk rebuild` with a space is not a script name — it is a `bzk` executable with a subcommand dispatcher, which is a CLI this project has never had and nobody has asked for. Every run on record used `python -m`; declaring a script would make thirty-odd sentences true about a thing that has never been executed, and would change what an install produces (§4.1) to do it. **What it costs to decide it this way is stated rather than glossed**: the shorthand appears in 37 places across the document set, and a reader who meets one of them without having read this paragraph can still type a command that does not exist. That is the price of the shorthand, and it is paid here rather than by rewriting 37 sentences into `python -m bzk.rebuild`, which would make the prose worse without making it truer.
...
within-run rate is not flat and the whole-run figure is the one to quote. **The instrument is
`python -m bzk.fetch_progress` since 2026-08-10 and was nowhere before that** — the three
pollers behind these figures were each rebuilt from memory, and what they counted had to be
recovered from prose. They counted the same thing: files arriving in `cache/uniprot/entry/`
and `cache/uniprot/seq/`. **The first reported per accession and the later two per round
trip, and the conversion was never stated beside them** — 5,273 trips over 2,260 accessions
is 2.3332, so *~1.0 s per accession* and *0.43 s per fetch* are one measurement and not a
2.5× disagreement. The module prints both units on every line for that reason. The third run was polled
...
`bzk drift` (`python -m bzk.drift`) leaves a receipt in `cache/uniprot/.drift`, and every `bzk rebuild` (`python -m bzk.rebuild`) reports how stale it is. **Rebuild never refuses on staleness** — it is the disaster-recovery path, and a network check standing in front of recovery would be worse than a stale check. The obligation that gives staleness teeth belongs at the export boundary with I18 and is recorded in `HANDOFF.md` §8; until it exists, the receipt is a report and not a control.
