...
| **A protein group cannot be ingested at protein grain — there is nowhere to record the ambiguity** | `bzk/adapters/perseus.py`, `ONTOLOGY.md` §6.3, I14 | A Perseus row naming `P19525;O43593` measured the *group*. Choosing one accession is the razor pick I14 forbids without a `confirmed` `ProteinAssignment` — and **§6.3 anchors `ProteinAssignment` on `SiteObservation`, not on `ProteinObservation`**, so at protein grain the model has no way to say "this measurement belongs to one of these three". `ProteinObservation` keys on its `Protein` anchor, so without a pick it cannot be minted at all. The adapter therefore refuses the file, naming the row, rather than taking the first accession or dropping the row. **This is likely to bite immediately**: MaxQuant's `Majority protein IDs` is semicolon-separated by construction and a Perseus table built from one inherits that, so a real export may be mostly groups. Two ways out and both are modelling decisions, not code: extend `ProteinAssignment` to anchor on `ProteinObservation` as well, or require group resolution upstream of ingestion. **Trigger: the collaborator's first real Perseus export** — the question to bring to the meeting is what fraction of its rows are groups. Yes for that file |
| **`perseus.py` is unvalidated against real input** | `bzk/adapters/perseus.py`, `raw/` | The adapter is written, tested and mutation-tested, entirely against synthetic fixtures — no Perseus export from the group exists. Three hazards are handled on the strength of the documented format rather than a real file: the `-Log Student's T-test p-value` column name, the `#!{...}` annotation rows, and `Majority protein IDs` as the accession column. All three are conventions, and a real export may spell any of them differently — the adapter refuses loudly on each (naming the columns it looked for) rather than guessing, so a mismatch is a clear error and not a silent empty result. **Trigger: the first real export.** Yes for end-to-end validation, no for the adapter existing |
...
| **Reserved-namespace collisions between other paired key spaces are unswept** | `bzk/curation/loader.py`, `data/curation/*.json`, ADR-0019 § reserved namespace | ADR-0019's rule is guarded for the one pair it was written from — the change-set format against the DDL. No other pair has been checked. **Known candidate: the curation record's key space against the DDL.** The loader maps record keys onto node columns by hand (`_DATASET_FROM_RECORD`, `_SAMPLE_FIELDS`), and the record also carries structural keys of its own — `pending`, `unresolved`, `corrections`, `mapping`, `contrasts_of_interest` — which no guard separates from field names. Today the mapping is explicit and small enough to read, so there is nothing to catch; that is the same position the change-set format was in for three ADR addenda. **Trigger: the second record format** — one hand-written mapping is an instance, two is a pattern, and an adapter reading a differently-shaped record is when a collision stops being hypothetical. Sweep then rather than guessing the shape now. No |
