...
command takes **37 m 14 s – 39 m 56 s**, *n* = 3 (`OPERATIONS.md` §5, where the timings live).
...
became a range — 37 m 14 s – 39 m 56 s, *n* = 3 after the third rehearsal — with the fetch count
reproducing exactly at 5,273 all three times; §5 carries both.
...
**Rehearsed cold a third time and the three trees are named — 2026-08-10.** The rehearsal reproduced
the live tree exactly: **15 labels, 14,134 ids, symmetric difference 0 on every label**, edge counts
identical, refusals 27, cache identical in file set, sequence bytes, pins and the same 7 `AMBIGUOUS`
snapshots, and **5,273** fetches for the third consecutive run. What it adds over the second is that
**the code moved and the output did not** — five landings separate the clone from the reference
graph, and I20 now runs at write time over all 1,362 results. Two figures widened: the cold rebuild
...
| ~~**`tests/test_agent_config_references.py` is unclassified in the tautology sweep — the suite is RED on that one module**~~ **Closed 2026-08-18** | `tests/test_tautology_sweep.py` `PINNED` l.203, floor l.1095 | The row's trigger was *"the next turn permitted to edit `tests/test_tautology_sweep.py`"*, and it fired. `('test_agent_config_references.py', 'len(docs) == EXPECTED_AGENT_DOCS', 1)` is classified **`PINNED`**, not `INSTANCES`: `len(docs)` is read off the filesystem by `_agent_docs()` and the right side is a hand-written literal, so neither side produced the other and there is no mutation to keep green. **The row argued that disposition and this turn established it rather than inheriting it** — the mutation adds one `.md` under `.claude/config/` carrying no code path, so the demonstration is isolated to one assert (`F..`, the other two green), and it fails at `tests/test_agent_config_references.py:116` — *that* assertion, not an earlier line — with *".claude/ holds 48 Markdown files, not 47"* and `assert 48 == 47`. The occurrence count was measured off `sweep()` rather than taken from the failure text, since `PINNED` is a multiset and a wrong count is a hole in the guard: one. The floor moved with it, **1123 → 1129 asserts and 31 → 32 modules** — both figures this time, because unlike the 2026-08-12 move the new tests landed in a module the sweep had never counted. Regeneration was run **after** classifying, not before: the observed set equals `PINNED` exactly at 144 entries, nothing unclassified and nothing stale, and all four `INSTANCES` rows still re-run green. Suite 534 passed / 1 failed → **535 passed, 11 skipped**. Resolved |
...
| ~~ADRs 0004–0014 unwritten~~ **Was never marked false: all eleven are written and this row went stale unnoticed, corrected 2026-08-31** | `decisions/`, `HANDOFF.md` l.225–226 | **Not a resolution — a correction.** Listing `decisions/` returns `0004-split-storage.md` through `0014-adapter-order.md`, all eleven present. l.225–226 of this file already recorded it on 2026-08-09 — *"The eight queued ADRs are written… 0018 is the only reserved number left"* — and this row was not struck at the same time. The struck rows above all record something that was **fixed**; nothing in this table records something that was **never true as stated**, so this form is new and is named as such rather than borrowed |
...
| **The sweep's re-run check cannot tell a caught mutation from an already-red tree** | `tests/test_tautology_sweep.py` l.1224, l.1001 | `test_every_classified_instance_re_runs_its_recorded_evidence` shells out to pytest and reads `result.returncode`, treating non-zero as evidence that a recorded instance now catches its defect. Against a tree already red for **any** unrelated reason the subprocess exits 1 whatever the mutation does, so every classified instance reports as no longer an instance and the failure message names a row that had nothing to do with the breakage. Found 2026-08-31 by a scratch edit to §3's anchors cell: two schema guards failed on the merits and this module failed third, naming a `test_drift.py` row. Control on the unedited tree: 1 passed. **Will surface on any turn that runs the sweep against a tree already failing** — which is every turn that breaks something and then runs the full suite |
