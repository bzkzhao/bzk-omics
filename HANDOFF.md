...
5. **The next action: get the first real `Observation` into the graph.** Everything upstream of it
   ...
   **What it needs, in order.** The MaxQuant *adapter* on top of `bzk/adapters/maxquant.py`'s
   reader — filtering, sample-name normalisation, `SiteObservation` construction; the resolver
   wired in, because `ModificationSite` anchors on a `ProteinSequence` and I2 requires its
   `sequence_version`; and `ModifierAssignment` on every site as `ambiguous` (§6.1, I3), which is
   the honest default for a K-GG remnant. Then `replay_ingestion` gains the adapter beside the
   loader and I9 stops being discharged for curation content only.
