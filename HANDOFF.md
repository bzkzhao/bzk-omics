...
3. ~~**The curation loader**~~ **Written 2026-08-07** — `bzk/curation/loader.py`, reading `data/curation/*.json` in the shape those files already have. Emits `Project → Experiment → Sample` plus the `Dataset` and the §5.3 curation `Analysis`, keyed through `bzk/ontology/keys.py`, validated against `invariants.validate` before it returns, and handed to an adapter as a `SampleMapping`. Both §8 loader defaults are applied (`parameters_observed = true`; no `Contrast` materialised). `bzk/adapters/perseus.py` is now the next action (Weeks 3–4 below).
   ...
   **The loader works; the PXD018299 record still does not load,** and the two facts are separate. It refuses with eight named items — the two titles, plus `Sample.timepoint_h` on the six unstimulated samples (a *third* blocker, found by writing the loader; see below). Verified directly: with `Sample.timepoint_h` classified in `schema.ABSENCE`, the same record refuses with exactly the two titles and nothing else.
...
3. **`Sample.timepoint_h` is null on the six unstimulated samples, and §3 classifies no such
   absence** — found 2026-08-07 by writing the loader, which is the point of writing it before the
   record is complete. `timepoint_h` is identifying on `Sample` (§3). The record's own `unresolved`
   states why it is null: *"Timepoint for the unstimulated (non-IFN) arms is not stated separately
   in the methods."* That is a **contingent** absence in ADR-0021's terms — the knowledge had not
   arrived, so the null describes the moment rather than the entity — and it is exactly what the
   classification forbids in an identifying position. It carries no `pending` marker, so only the
   loader's second layer catches it.

   Two ways out, and **both are the curator's, not a session's.** Either the timepoint for the
   unstimulated arms is supplied (48 h is *likely*, since the IFN arms were harvested then, but
   likely is not stated — assuming it is the failure mode this whole scheme exists to prevent), or
   §3 gains a row classifying the absence `determined` **by `treatment`**, on the argument that an
   untreated arm has no treatment timepoint. The second is defensible modelling, but §3 requires a
   `determined` classification to name what forces the null, and the record currently says the
   opposite — that the value exists and was not reported. Writing that row without resolving the
   contradiction would be asserting what the data does not support.
...
`cell_line`) without stating it. The third is a different shape — not a field the format forgot, but
a value the source publication never gave.
