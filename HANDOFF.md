...
| **`tests/test_agent_config_references.py` is unclassified in the tautology sweep — the suite is RED on that one module** | `tests/test_tautology_sweep.py` `PINNED`, `tests/test_agent_config_references.py` | **Blocking the suite, not the work.** The new guard over `.claude/`'s pointers added one match to `tests/test_tautology_sweep.py`'s sweep: `('test_agent_config_references.py', 'len(docs) == EXPECTED_AGENT_DOCS', 1)`. The sweep requires every match in `tests/` to be classified into `PINNED` or `INSTANCES`, both of which are defined **inside `tests/test_tautology_sweep.py`** — a file the turn that wrote this row was told by name not to edit. So the match is real, the classification is a two-line edit, and the edit was not permitted. **It is not a tautology**: `docs` is read off the filesystem and `EXPECTED_AGENT_DOCS` is a pinned literal, and mutation C (copying one file into `.claude/config/`) makes the assertion fail with the count in the message — the same shape `tests/test_decision_index.py` already carries and which is already classified. **It was not reformulated to dodge the classifier**, though it could have been in one character: Pass C excludes comparisons against a literal display, so `len(docs) == 47` never matches while `len(docs) == EXPECTED_AGENT_DOCS` does. Writing the literal would have bought a green suite by evading the review the sweep exists to force, which is the same move as downgrading an invariant to make a dataset load. **Trigger: the next turn permitted to edit `tests/test_tautology_sweep.py`** — add the tuple above to `PINNED`, then `pytest` is green. Until then `pytest` reports one failure in `test_the_pinned_multiset_has_not_changed_unreviewed` and every other module passes |
